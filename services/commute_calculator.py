...
以下の3つの交通手段について、実際のルートに基づいて通勤情報を推定してください。
数値は現実的な概算で構いません。
...
{{
  "car": {{
    "minutes": <片道所要時間（分）>,
    "distance_km": <片道距離（km）>,
    "route": "<主な経路（例: 東北自動車道 郡山IC→浦和IC など）>",
    "cost_yen": <片道高速代の概算（円）>,
    "feasibility": "<daily | occasional | impractical のいずれか>"
  }},
  "shinkansen": {{
    "minutes": <片道所要時間（分）>,
    "distance_km": <片道距離（km）>,
    "route": "<主な経路（例: 郡山→東京(新幹線75分)+山手線(20分) など）>",
    "cost_yen": <片道運賃概算（円）>,
    "feasibility": "<daily | occasional | impractical のいずれか>"
  }},
  "train": {{
    "minutes": <片道所要時間（分）。在来線のみで非現実的な場合は null>,
    "distance_km": <片道距離（km）。不明なら null>,
    "route": "<経路の説明>",
...
- impractical: 通勤は非現実的（転居または完全リモート必須）"""
